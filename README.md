...
- 重要な操作はメニューバーからも実行できます。
  - `設定保存 / 設定読込 / ジョブ保存 / ジョブ読込 / ドライラン / 一括実行 / キャンセル / ログ保存`
- 画面中央はタブ構成です。
  - `キュー`
  - `設定`
  - `単体モード`
  - `ログ`
- 下側が見切れても、最低限の基本操作はメニューバーから辿れます。
...
- 出力先制御
  - 出力ルートフォルダ指定
  - 既定値は `出力ルートへまとめて出す`
  - まとめ出力時の既定ファイル名は `{video_name}_{frame_no}`
  - 必要に応じて `{動画名}_frames` 自動サブフォルダへ切り替え可能
...
1. 上段の `入力動画キュー` に動画を追加します。
   - ドラッグ＆ドロップ時にすでにキューがある場合は、`一覧をクリアして入れ替える / 後ろに追加する / 何もしない` を選べます。
2. `出力ルートフォルダ` を指定します。
3. `バッチ出力方法` を確認します。
   - 既定値: `出力ルートへまとめて出す`
   - この場合の既定ファイル名: `{video_name}_{frame_no}`
   - 後で自分で仕分けしたい時は、この既定値のまま使うのが楽です。
4. 必要なら `出力ファイル名パターン` を変更します。
   - 既定値: `{video_name}_{frame_no}`
   - 例: `{video_name}_{timestamp}_{frame_no}`
   - `出力ルートへまとめて出す` 時は `{video_name}` を含めてください。
5. `サブフォルダ重複時` と `既存出力の扱い` を確認します。
6. `ドライラン` で予定を確認します。
7. 問題なければ `一括実行` を押します。
8. `一時停止` は現在動画の完了後に停止します。
9. `再開` は未処理だけを続きから処理します。
10. `キャンセル` は現在の `ffmpeg` を止め、残りをキャンセル扱いにします。

## aspectfix GUI

- 起動: `run_aspectfix_dragdrop.bat`
- 基本ウィンドウサイズは `1280x1024` です。
- メニューバーから `設定保存 / 設定読込 / ファイル追加 / フォルダ追加 / 処理開始 / 停止要求 / 出力先を開く` を実行できます。
...
- 起動: `run.bat`
- 基本ウィンドウサイズは `1280x1024` です。
- メニューバーから `設定保存 / 設定読込 / 処理開始 / 停止要求 / 出力先を開く / README を開く` を実行できます。
...
- `既存出力があればスキップ`
  - `出力ルートへまとめて出す` 時は、同じ動画に対応する既存画像があれば、その動画は処理しません。
  - `動画ごとにサブフォルダを作る` 時は、既に出力済みのフォルダがあれば、その動画は処理しません。
- `既存出力があれば連番フォルダを作る`
  - `出力ルートへまとめて出す` 時は、既存結果を残したまま、重複したファイル名だけ `_001`, `_002` を付けて保存します。
  - `動画ごとにサブフォルダを作る` 時は、既存結果を残したまま、新しい `_001`, `_002` フォルダへ出します。
- `既存出力を消して作り直す`
  - `出力ルートへまとめて出す` 時は、同じ動画に対応する既存画像だけを削除して再作成します。
  - `動画ごとにサブフォルダを作る` 時は、対象フォルダを削除して再作成します。
  - どちらも明示的に選んだ場合だけ削除確認を出します。
